...
public class GetBushFromXML {
	public Set<Plant> parseBushXML() throws ParserConfigurationException, SAXException, IOException {
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				plantBush=new Bush(name, hight);
				int rand = new Random().nextInt(5);
				plantBush.setLeafType(LeafType.values()[rand]);
				plantBush.setFruit(eElement.getElementsByTagName("fruit").item(0).getTextContent());
				
				// Add Employee to list
				listOfPlants.add(plantBush);
			}
		}
		return listOfPlants;
	}
